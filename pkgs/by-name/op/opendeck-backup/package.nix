...
let
  version = "b2d1ab322e1cae63ba3f0b46f02d34448896667d"; # v2.7.1
  ...
  frontend = stdenv.mkDerivation {
    ...
    outputHash = "sha256-/WJuqY4gQ0zOCOYSzkSBcw9KWOjMEcSMWwOTytsouQ4=";
  ...
  pluginDenoDeps = stdenv.mkDerivation {
    ...
    # Make this a Fixed Output Derivation since we need to allow network access for Deno to download dependencies
    outputHashMode = "recursive";
    outputHash = "sha256-GiKpWIyQnYL+NismE8YEc3ZK0leWXx7DzZ42kIDSDBo=";
  ...
  plugins = stdenv.mkDerivation {
    ...
    # Copy the plugin's Cargo.lock
    postUnpack = ''
      cp "$sourceRoot/plugins/com.amansprojects.starterpack.sdPlugin/Cargo.lock" "$sourceRoot/"
    '';

    # Use importCargoLock with outputHashes for git dependencies (same as main app)
    ...
    buildPhase = ''
      ...
      # Inject Cargo.lock into vendored enigo (it doesn't have one at this revision)
      VENDOR_DIR="/build/cargo-vendor-dir"
      if [ -d "$VENDOR_DIR" ]; then
        ENIGO_DIR=$(find "$VENDOR_DIR" -maxdepth 1 -type d -name "enigo-*" 2>/dev/null | head -1)
        if [ -n "$ENIGO_DIR" ] && [ -d "$ENIGO_DIR" ]; then
          echo "Injecting Cargo.lock into vendored enigo at $ENIGO_DIR"
          cp ${./enigo-Cargo.lock} "$ENIGO_DIR/Cargo.lock"
          chmod +w "$ENIGO_DIR/Cargo.lock"
          if [ -f "$ENIGO_DIR/Cargo.lock" ]; then
            echo "✓ Cargo.lock successfully created in enigo"
          else
            echo "✗ Failed to create Cargo.lock in enigo!"
            exit 1
          fi
        else
          echo "Warning: Could not find vendored enigo directory in $VENDOR_DIR"
        fi
      else
        echo "Warning: Vendor directory $VENDOR_DIR does not exist"
      fi
